...
public class MainActivity extends Activity {
    ...
    /**
     * Keep the native entry layer stable.
     * The background image stays still, and only the crucifix layer appears gently.
     * Timing of overlay removal is kept the same as before; this is visual-only and does
     * not touch URL, location, back, or re-entry behavior.
     */
    private void addLaunchOverlay() {
        ...
        bg.setImageResource(getResources().getIdentifier("launch_intro_bg", "drawable", getPackageName()));
        bg.setScaleType(ImageView.ScaleType.CENTER_CROP);
        launchOverlay.addView(bg, new FrameLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT
        ));

        ImageView cross = new ImageView(this);
        cross.setImageResource(getResources().getIdentifier("launch_intro_cross", "drawable", getPackageName()));
        cross.setScaleType(ImageView.ScaleType.CENTER_CROP);
        cross.setAlpha(0f);
        cross.setScaleX(0.985f);
        cross.setScaleY(0.985f);
        launchOverlay.addView(cross, new FrameLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT
        ));
        cross.animate()
