...
(function(){
  ...
  function armCoverBackTrap(reason, opts){
    /* V2-S-8P: patches.js를 커버 뒤로가기 trap 생성의 최종 기준으로 둔다.
  ...
  /* history 초기화
     V2-S-8P: 첫 커버 뒤로가기 실패를 만들던 hash/query trap 흔적을 제거하고,
  ...
  /* ─────────────────────────────────────────────
     V2-S-8P 기도문 전용 뒤로가기 컨트롤러 — history 단계 분리 제거
  ...
  function armPrayerBackTrap(reason){
    /* 호환용 함수. V2-S-8P부터 기도문 detail/list용 별도 pushState는 만들지 않는다.
  ...
  function settleCoverTrapAfterPrayer(reason){
    // V2-S-8P: 기도문 팝업 → 커버 후에는 이미 공통 컨트롤러가 history.go(1)로 trap을 복원한 상태다.
  ...
  function prayerListToPopupOrCover(reason){
    try{
      ...
      /* V2-S-8P: 기도문 목록 → 빠른메뉴 팝업 복귀는 직접 팝업을 띄우지 않는다.
...
(function(){
  ...
  // V2-S-8P: 커버 글자 크기 조절은 prayer.js에 의존하지 않는 공통 함수가 담당한다.
  // prayer.js는 기도문 화면이 열렸을 때 같은 localStorage 값을 읽어 자체 UI를 맞춘다.
  var QA_URL="qa-firebase.html?v=V2-S-8P";
  ...
  function configureQna(){
    // V2-S-8P: 문의·건의 버튼은 중간 안내 카드를 만들지 않고 실제 문의 페이지로 바로 이동한다.
