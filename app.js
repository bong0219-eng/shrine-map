...
function _runRefreshAppFilesOnly(){
  ...
  try{
    if(btn){
      btn.disabled = true;
      btn.textContent = ((btn.getAttribute('data-target-version') || 'V1-26') + ' 새로고침 중');
    }
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
        var target = btn.getAttribute('data-target-version') || 'V1-26';
    var current = '';
    ...
    if(marker){
      marker.textContent = target || 'V1-26';
...
function goToCover(){
  try{
    if(document.querySelector('#web-view.open,#trail-view.open,#qna-view.open,#diocese-view.open,#missa-view.open') && typeof oaiHoldStabilityVeil === 'function') oaiHoldStabilityVeil('view-close', 80);
  }catch(e){ console.warn('[가톨릭길동무]', e); }
  closeTab(_activeTab);
  closeInfoCard();
  resetRoute();
...
document.addEventListener('DOMContentLoaded', function bindEvents() {
  ...
  (function(){
    ...
    function openCoverMenuPageFast(url, flagName){
      try{ closeMenu(); }catch(_e){}
      try{ if(flagName){ sessionStorage.setItem(flagName, '1'); localStorage.setItem(flagName, '1'); } }catch(_e){}
      try{
        if(typeof window._oaiNormalizeCoverBackAfterReturn === 'function') window._oaiNormalizeCoverBackAfterReturn('before-page-open');
        else if(typeof window._oaiArmCoverBackTrap === 'function') window._oaiArmCoverBackTrap('before-page-open', {force:true});
      }catch(e){ console.warn('[가톨릭길동무]', e); }
    ...
    on('cover-menu-guide-link', 'click', function(e){ if(e && e.preventDefault) e.preventDefault(); openCoverMenuPageFast('guide.html', 'oai_returned_from_guide'); });
    on('cover-menu-qna-btn', 'click', function(e){
      if(e && e.preventDefault) e.preventDefault();
      closeMenu();
      try{ openQnaView(); }catch(err){ console.warn('[가톨릭길동무]', err); }
    });
    on('cover-menu-privacy-link', 'click', function(e){ if(e && e.preventDefault) e.preventDefault(); openCoverMenuPageFast('privacy.html', 'oai_returned_from_privacy'); });
    document.addEventListener('keydown', function(e){
