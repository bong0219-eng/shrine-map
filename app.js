...
function closeMassQuickMenu(){
  ...
  try{
    ...
    const toast=document.getElementById('_bt');
...
document.addEventListener('DOMContentLoaded', function bindEvents() {
  ...
  on('mass-quick-missa', 'click', function() { _setMassQuickReturn(false); if (typeof openMissa === 'function') openMissa(); });
  on('mass-quick-prayer', 'click', function() { _setMassQuickReturn(false); window.__MASS_QUICK_FROM_PRAYER__ = false; var modal=document.getElementById('mass-quick-modal'); if(modal){ modal.classList.remove('show'); modal.setAttribute('aria-hidden','true'); } hideCoverAndRun(function() { if (typeof openPrayerBook === 'function') openPrayerBook(); else alert('기도문 기능이 연결되지 않았습니다.'); }); });
  on('mass-quick-hymn', 'click', function() { _setMassQuickReturn(false); if (typeof openCatholicHymn === 'function') openCatholicHymn(); });
