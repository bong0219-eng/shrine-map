...
// V2-S-r26: 긴 캐시 초기화 후 붙는 ?refresh=...는 파일 재요청용 표식일 뿐
...
function markExternalReturnStabilize(kind){
  // V2-S-r26: 외부 사이트 이동 직전 보호창을 시작한다.
...
function oaiSmoothNavigate(url, kind){
  // V2-S-r26: 각 카테고리의 기존 외부 이동 구조는 유지하되,
...
function openPrayerBook(opts){
  ...
  // V2-S-r26: restore 변수 미정의 오류 방지. 주요기도문 초기화가 중간에 끊기면
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V2-S-r26';
  }else if(!restore){
...
function normalizeCatholicExternalUrl(url){
  ...
  try{
    ...
    // V2-S-r26: 원주·인천교구 대표 홈페이지는 공식 등록 주소가 HTTP이므로
...
function openDioceseExternal(url, state){
  ...
  // V2-S-r26: 관구·교구 홈페이지도 다른 외부 사이트와 같은 보호창 흐름을 사용한다.
...
function restoreDioceseExternalState(opts){
  ...
  try{
    ...
    // V2-S-r26 stable: frame.contentWindow가 있다는 이유만으로 '살아 있다'고 판단하면 안 된다.
...
// V2-S-r26: 성당 데이터를 교구별 parishes-*.js 파일로 실제 분리한다.
...
const _PARISH_ASSET_VERSION='V2-S-r26';
...
const _PRAYER_ASSET_VERSION='V2-S-r26';
...
const _RETREAT_ASSET_VERSION='V2-S-r26';
...
const _SHRINE_ASSET_VERSION='V2-S-r26';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function oaiEnterView(el){
  ...
  try{
    ...
    // V2-S-r26: 화면 진입 효과는 안정막으로 덮지 않고, 실제 화면 자체를 아주 짧게 fade-in 한다.
...
function _onMapReady(){
 const KM=kakao.maps;window._LL=KM.LatLng;window._MM=KM.Marker;window._MI=KM.MarkerImage;window._SZ=KM.Size;window._PT=KM.Point;window._LB=KM.LatLngBounds;window._PL=KM.Polyline;window._EL=KM.event.addListener;
  ...
  if(!window._noAutoNearby){
    // V2-S-r26: 성당 첫 진입도 기존 기준대로 내주변 탭을 먼저 연다.
...
function _buildParishDioSystem(){
  ...
  Object.entries(_DIO_CFG).forEach(([code,cfg])=>{
    // V2-S-r26: 군종교구는 데이터/검색에는 남기되 지도 위 교구 라벨에서는 제외한다.
    if(code==='ML') return;
...
function renderList(){
  ...
  if(q){
    ...
    let matchAll=false;
    if(_mode==='parish'){
      /* V2-S-r26: 성당찾기는 선택한 교구 안에서 성당명 첫 글자 일치 또는 주소 포함으로만 찾는다. */
...
function filterModal(q){
  ...
  if(q){
    ...
    let matchAll=false;
    if(_mode==='parish'){
      /* V2-S-r26: 성당 길찾기 검색도 선택한 교구 안에서 성당명 첫 글자 일치 또는 주소 포함으로만 찾는다. */
      matchAll = nameNorm.startsWith(nq) || addrNorm.includes(nq);
    } else {
      const tokens=q.trim().split(/\s+/);
      matchAll=tokens.length>=2
