...
function oaiPrepareRefreshVeil(reason, duration, carryDuration, showBeforeNavigation, beforeNavigationHold, carryToNextDocument){
  try{
    ...
    var now = Date.now ? Date.now() : new Date().getTime();
    /*
       V2-S-2-2: 수동 짧은/긴 새로고침은 OK 직후 현재 문서 보호막을 먼저 보여 주되,
...
async function _runClearAppFilesCacheCompletely(){
  ...
  // V2-S-2-2: 긴 새로고침은 현재 문서 보호막 하나만 사용한다.
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || 'V2-S-2-2';
    var current = '';
...
function openPrayerBook(opts){
  ...
  // V2-S-2-2: restore 변수 미정의 오류 방지. 주요기도문 초기화가 중간에 끊기면
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V2-S-2-2';
  }else if(!restore){
...
function normalizeCatholicExternalUrl(url){
  ...
  try{
    ...
    // V2-S-2-2: 원주·인천교구 대표 홈페이지는 공식 등록 주소가 HTTP이므로
...
function restoreDioceseExternalState(opts){
  ...
  try{
    ...
    // V2-S-2-2 stable: frame.contentWindow가 있다는 이유만으로 '살아 있다'고 판단하면 안 된다.
...
// V2-S-2-2: 성당 데이터를 교구별 parishes-*.js 파일로 실제 분리한다.
...
const _PARISH_ASSET_VERSION='V2-S-2-2';
...
const _PRAYER_ASSET_VERSION='V2-S-2-2';
...
const _RETREAT_ASSET_VERSION='V2-S-2-2';
...
const _SHRINE_ASSET_VERSION='V2-S-2-2';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function _showBackToast(){
  ...
  _armCoverExitWindow();
  try{
    // V2-S-2-2: 커버가 이미 보이는 상태에서 Back 안내를 띄울 때는
    // 새로고침/외부복귀 안정막이 남아 있더라도 안내 문구가 그 아래에 숨지 않게 한다.
    document.documentElement.classList.remove('oai-stability-veil','oai-external-return-freeze','oai-external-leaving','oai-stability-veil-releasing');
  ...
  const old=$('_bt');
  if(old) old.remove();
  const msg='한 번 더 누르면 앱이 종료됩니다';
  const t=document.createElement('div');
  t.id='_bt';
  t.setAttribute('role','status');
  t.textContent=msg;
  t.style.cssText='position:fixed;top:50%;left:50%;bottom:auto;transform:translate(-50%,-50%);background:rgba(14,21,53,.94);color:#fff;padding:12px 24px;border-radius:24px;font-size:14px;font-weight:800;z-index:2147483647;white-space:nowrap;pointer-events:none;box-shadow:0 14px 36px rgba(0,0,0,.32);';
  document.body.appendChild(t);
  try{
    const t2=$('oai-cover-exit-toast');
    if(t2){
      t2.textContent=msg;
      t2.style.zIndex='2147483647';
  ...
  _exitTimer=setTimeout(function(){
    ...
    try{ const t2=$('oai-cover-exit-toast'); if(t2) t2.classList.remove('show'); }catch(e){ console.warn('[가톨릭길동무]', e); }
  },2500);
...
function oaiEnterView(el){
  ...
  try{
    ...
    // V2-S-2-2: 카테고리 진입은 화면 자체를 fade하지 않고, 완성된 화면 위의
...
function _onMapReady(){
 const KM=kakao.maps;window._LL=KM.LatLng;window._MM=KM.Marker;window._MI=KM.MarkerImage;window._SZ=KM.Size;window._PT=KM.Point;window._LB=KM.LatLngBounds;window._PL=KM.Polyline;window._EL=KM.event.addListener;
  ...
  if(!window._noAutoNearby){
    // V2-S-2-2: 성당 첫 진입도 기존 기준대로 내주변 탭을 먼저 연다.
...
function _buildParishDioSystem(){
  ...
  Object.entries(_DIO_CFG).forEach(([code,cfg])=>{
    // V2-S-2-2: 군종교구는 데이터/검색에는 남기되 지도 위 교구 라벨에서는 제외한다.
    if(code==='ML') return;
...
function renderList(){
  ...
  if(q){
    ...
    let matchAll=false;
    if(_mode==='parish'){
      /* V2-S-2-2: 성당찾기는 선택한 교구 안에서 성당명 첫 글자 일치 또는 주소 포함으로만 찾는다. */
...
function filterModal(q){
  ...
  if(q){
    ...
    let matchAll=false;
    if(_mode==='parish'){
      /* V2-S-2-2: 성당 길찾기 검색도 선택한 교구 안에서 성당명 첫 글자 일치 또는 주소 포함으로만 찾는다. */
      matchAll = nameNorm.startsWith(nq) || addrNorm.includes(nq);
    } else {
      const tokens=q.trim().split(/\s+/);
      matchAll=tokens.length>=2
